...
impl MapBuilder {
	...
	fn find_most_distant(&self) -> Point {
		...
		self.map.index_to_point2d(dijkstra_map.map
				.iter()
				.enumerate()
	...
	fn spawn_monsters(&self, start: &Point, rng: &mut RandomNumberGenerator) -> Vec<Point> {
		...
		let mut spawnable_tiles : Vec<Point> = self.map.tiles
			.iter().enumerate().
			filter(|(idx, t)|
				**t == TileType::Floor && DistanceAlg::Pythagoras
					.distance2d(*start, self.map.index_to_point2d(*idx)) > 10.0)
			.map(|(idx, _)| self.map.index_to_point2d(idx)).collect();
